...
def create_adjacency_matrix( neuronsA, neuronsB, remote_instance, syn_cutoff = None, row_groups = {}, column_groups = {} ):
	""" Wrapper to generate a matrix for synaptic connections between neuronsA -> neuronsB (unidirectional!)
...
def create_connectivity_distance_matrix( neurons, remote_instance, upstream=True, downstream=True, threshold=1, filter_skids=[], exclude_skids=[], plot_matrix = True):
	""" Wrapper to calculate connectivity similarity and creates a distance matrix for a set of neurons
	...
	threshold :			int (default = 1)
						Only partners with >= this synapses are considered. Attention: this might impair proper comparison:
						e.g. neuronA and neuronB connect to neuronC with 1 and 3 synapses, respectively. 
						If threshold = 2, then connection from A to C will be ignored!
	filter_skids :		list of skids
