...
def geodesic_matrix(x):
    ...
    """
    if x.igraph:
        mat = x.igraph.shortest_paths_dijkstra(mode='All', weights='weight')
        nodes = [ n['node_id'] for n in x.igraph.vs ]
    else:
        # Fall back to networkx (much slower here)
        # Make generator for pairwise distances: 
        # each item is a tuple with [0] = source and [1] = dict of targets
        gen = nx.all_pairs_dijkstra_path_length( x.graph.to_undirected(as_view=True), weight='weight')

        dist = [ d for d in tqdm( gen, total= x.n_nodes ) ]

        nodes = [ d[0] for d in dist ]

        mat = [ [ d[1][n] for n in nodes ] for d in dist ]

    mat = np.array(mat, dtype=int )

    return pd.DataFrame(mat, columns=nodes, index=nodes, dtype=int )
...
def cut_neuron(x, cut_node, g=None):
    """ Uses network X to split neuron at given point and returns two new neurons.
